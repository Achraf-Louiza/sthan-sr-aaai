...
def trr_loss_mse_rank(pred, base_price, ground_truth, mask, alpha, no_stocks):
    ...
    all_ones = torch.ones(no_stocks,1)
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            for j in tqdm(range(self.valid_index - self.parameters['seq'] -self.steps +1)):
                ...
                output = model(torch.FloatTensor(emb_batch), hyp_input)
                cur_loss, cur_reg_loss, cur_rank_loss, curr_rr_train = trr_loss_mse_rank(output.reshape((1026,1)), torch.FloatTensor(price_batch), 
            ...
            with torch.no_grad():
                ...
                for cur_offset in range(
                    ...
                    output_val = model(torch.FloatTensor(emb_batch), hyp_input)
                    cur_loss, cur_reg_loss, cur_rank_loss, cur_rr = trr_loss_mse_rank(output_val, torch.FloatTensor(price_batch), 
                                                                                            torch.FloatTensor(gt_batch), 
                                                                                            torch.FloatTensor(mask_batch), 
                                                                                            self.parameters['alpha'], self.batch_size)
                ...
                for cur_offset in range(self.test_index - self.parameters['seq'] - self.steps + 1,
                    ...
                    output_test = model(torch.FloatTensor(emb_batch), hyp_input)
                    cur_loss, cur_reg_loss, cur_rank_loss, cur_rr = trr_loss_mse_rank(output_test, torch.FloatTensor(price_batch), 
                                                                                            torch.FloatTensor(gt_batch), 
                                                                                            torch.FloatTensor(mask_batch), 
                                                                                            self.parameters['alpha'], self.batch_size)
